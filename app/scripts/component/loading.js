...
define(['jquery', './maskLayer'], function ($, maskLayer ) {
    ...
    loading_.prototype = {
        ...
        create: function () {
            ...
            }).append($('<img src="' + this.config.url + '"/>'));
    ...
    return function (config) {
        ...
        config = $.extend({
            url: '/assets/images/loadding.gif',
            isMask: true,
            width: 124,
            height: 124,
        ...
        loadObj = $(config.wrapBox).data('data-loading');

        if (!loadObj) {
            $(config.wrapBox).data('data-loading', loadObj = new loading_(config)) ;
        }
        return loadObj;
    };
